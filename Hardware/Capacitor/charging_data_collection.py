...
full_power_duration_ms = 30000  # 35 seconds
...
while True:
    if first_run:
        ...
        # ⬇️ Reduce sample rate to 500 Hz (2 ms intervals)
    ...
    if timer_elapsed == 1:
        ...
        if full_power_mode:
            if time.ticks_diff(current_time, full_power_start_time) < full_power_duration_ms:
                i_ref = 0.25  # Constant current during logging
            else:
        ...
        if not full_power_mode and not power_reported:
            if power_sample_count > 0:
                ...
                print("Average Output Power: {:.3f} W".format(avg_power))
                print("-----------------------------------\n")
            else:
